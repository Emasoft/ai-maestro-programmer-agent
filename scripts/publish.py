...
# script. publish.py sets AI_MAESTRO_PUBLISH_AUTHORIZED=1 in the environment
# immediately before calling `git push`. The hook checks that variable and
# aborts the push if it is not set.
#
# This closes the loop: even if someone tries to `git push` manually after
# editing files by hand, the hook refuses because AI_MAESTRO_PUBLISH_AUTHORIZED
# is only set inside a publish.py run where every validation step has already
...
def ensure_pre_push_hook(git_root: Path) -> None:
    ...
    hook itself will not fire on THIS publish.py run because the run sets
    AI_MAESTRO_PUBLISH_AUTHORIZED=1 just before the push — but any future
    manual `git push` will be refused until publish.py runs again.
    """
...
Examples:
  ...
  %(prog)s --patch --dry-run    # run every validation step fully, then
    ...
    # Set the authorization sentinel IMMEDIATELY before push. The pre-push
    # hook reads this env var and refuses the push if it's not set. We set
    # it here, AFTER every validation step has passed — so there is no path
    # where `git push` happens without a full strict validation run first.
    print(f"\n{BLUE}=== Step 13: Push commit + tag to origin/{default_branch} ==={NC}")
    os.environ["AI_MAESTRO_PUBLISH_AUTHORIZED"] = "1"
    try:
        run(["git", "push", "origin", "HEAD"], cwd=git_root)
        run(["git", "push", "origin", f"v{new_version}"], cwd=git_root)
    finally:
        os.environ.pop("AI_MAESTRO_PUBLISH_AUTHORIZED", None)
