...
description: Create handoff documents for context transfer
...
This operation explains how to create handoff documents that transfer work
context to other agents or preserve state for future sessions.

## When to Use

Use this operation when:

1. **Completing a delegated task**: You finished the work and need to return
   results to AMOA
...
5. **Blocked and escalating**: You cannot proceed and need to hand off to a
   different agent

Do NOT use this operation when:

- You are in the middle of active implementation (use Document Work State
  instead)
...
1. **Know the recipient**: Either a specific agent name or "future self" for
   session resume
...
4. **Handoff directory must be writable**:
...
<One paragraph describing what the task is about, what was accomplished, and
what remains.>
```
...
**Completed**: 2 of 4 requirements **Remaining**: Requirements 3 and 4
```
...
**Task:** <task-name> **Started:** <ISO-timestamp-when-work-began> **Last
...
- **Content**: include the handoff document file path, the current status, and a
  brief summary of the work state
- **Type**: handoff
...
- [ ] Created task directory at
      `$CLAUDE_PROJECT_DIR/thoughts/shared/handoffs/<task>/`
...
Implemented a YAML configuration parser with schema validation. All requirements
met, all tests passing, code reviewed and documented.
...
1. Check if parent directory exists:
   `ls -la $CLAUDE_PROJECT_DIR/thoughts/shared/`
2. Create the directory structure:
   `mkdir -p $CLAUDE_PROJECT_DIR/thoughts/shared/handoffs`
3. Verify write permissions
4. If running in a sandboxed environment, request the user to create the
   directory
...
**Cause**: Two agents were assigned the same task, or the previous handoff was
...
3. If the other agent is no longer active, archive their handoff and create
   yours
